...
from app.utils.chroma_db import add_to_chroma
...
def create_module(
    ...
    # CREATE MODULE (correct position)
    db_module = crud.create_module(db, module)

    # ADD TO CHROMADB
    add_to_chroma(
        doc_id=str(db_module.id),
        text=f"{db_module.title} {db_module.description}"
    )
